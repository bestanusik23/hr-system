...
// POST   /api/exec/license-exclusions   body: {employee_id}  → exclude from the license KPI
// DELETE /api/exec/license-exclusions?employee_id=           → include back
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  await ctx.env.HR_DB.prepare(
    "INSERT INTO iso_kpi_license_exclusions (employee_id, excluded_by) VALUES (?,?) ON CONFLICT(employee_id) DO NOTHING"
...
export const onRequestDelete: PagesFunction<Env> = async (ctx) => {
  ...
  await ctx.env.HR_DB.prepare("DELETE FROM iso_kpi_license_exclusions WHERE employee_id = ?").bind(employeeId).run();
  return Response.json({ ok: true });
};
